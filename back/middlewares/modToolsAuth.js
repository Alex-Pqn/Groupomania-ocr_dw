...
module.exports = (req, res, next) => {
    try {
        ...
        db.query(`SELECT is_mod, is_admin FROM Accounts WHERE id=${ userId }`, (err, result) => {
            if(err) {
                return res.status(400).json({ sub_err: "L'authentification avec la base de donnée a échouée, veuillez réessayer dans quelques instants.", err })
            }
        ...
        res.status(401).json({ sub_err:"Vous n'avez pas accès aux outils de modération.", err });
    }
  };