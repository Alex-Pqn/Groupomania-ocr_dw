    ...
    // data validation success
      ...
      bcrypt
        ...
        .then(function (hashed_password) {
          ...
          db.query(
            `INSERT INTO user (firstname, lastname, email, hashed_password, newsletters) VALUES ('${firstname}', '${lastname}', '${email}', '${hashed_password}', ${newsletters});`,
            (err, result) => {
              let userDb = result;
              ...
              if (err) {
                ...
                if (err.errno === 1062) {
                  return res.status(400).json({
                    sub_err:
                      "Il semblerait qu'un compte possédant cet e-mail est déjà existant.",
                    err,
                  });
                }
                // other
                  ...
                  return res.status(400).json({
                    sub_err:
                      'La création de votre compte a échoué, veuillez réessayer dans quelques instants.',
                    err,
                  });
                }
              }
...
exports.login = (req, res, next) => {
  ...
  if (email && password) {
    ...
    if (dataValidation.error) {
      ...
      return res.status(400).json({
        sub_err:
          "Validation de donnée: Il semblerait que l'un des champs requis est manquant ou incorrect.",
        err: dataValidationError,
      });
    }
    // data validation success
      ...
      db.query(
        ...
        (err, result) => {
          ...
          if (err) {
            return res.status(400).json({
              sub_err:
                "La tentative d'authentification a échoué, veuillez réessayer dans quelques instants.",
              err,
            });
          }
          ...
          if (result.length === 0) {
            return res.status(401).json({
              sub_err: "L'e-mail et le mot de passe entré sont incorrects.",
              err,
            });
          }
          ...
          // account finded
            ...
            // compare passwords
            bcrypt
              .compare(password, userDb.hashed_password)
              .then(function (bcryptResult) {
                // passwords don't matchs
                if (!bcryptResult) {
                  return res.status(401).json({
                    sub_err:
                      "L'e-mail et le mot de passe entré sont incorrects.",
                  });
                }
                // passwords matchs
                else {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        if (err) {
          return res.status(400).json({
            sub_err:
              "La récupération des données de l'utilisateur a échouée, veuillez réessayer dans quelques instants.",
            err,
          });
        }
    ...
    db.query(
      ...
      (err, result) => {
        ...
        if (err) {
          return res.status(400).json({
            sub_err:
              "La récupération des paramètres de l'utilisateur a échouée, veuillez réessayer dans quelques instants.",
            err,
          });
        }
  ...
  if (firstname && lastname && email && newsletters && pic_url && userId) {
    ...
    // data validation success
      ...
      if (req.file) {
        ...
        pic_url = `${req.protocol}://${req.get('host')}/images/${
          req.file.filename
        }`;
      }
      ...
      if (password) {
        bcrypt
          ...
          // hash password success
          .then(function (hashed_password) {
            dbQuery = `UPDATE user SET pic_url="${pic_url}", firstname="${firstname}", lastname="${lastname}", description="${description}", email="${email}", hashed_password="${hashed_password}", newsletters=${newsletters} WHERE id = ${userId}`;
            updateParameters();
          })
          ...
          .catch((err) =>
            res.status(500).json({
              sub_err:
                'La modification des paramètres utilisateur a échouée, veuillez réessayer dans quelques instants..',
              err,
            })
          );
      }
      // if there is not password in request
        ...
        dbQuery = `UPDATE user SET pic_url="${pic_url}", firstname="${firstname}", lastname="${lastname}", description="${description}", email="${email}", newsletters=${newsletters} WHERE id = ${userId}`;
        updateParameters();
      }
...
exports.deleteAccount = (req, res, next) => {
  ...
  if (userId) {
    ...
    db.query(`SELECT * FROM forum WHERE user_id=${userId}`, (err, result) => {
      // error handler
      if (err) {
        return res.status(400).json({
          sub_err:
            'La récupération de vos discussions et la suppression de votre compte a échouée, veuillez réessayer dans quelques instants.',
          err,
        });
      }
      ...
      if (userForumsList.length >= 1) {
        dbQuery = `DELETE FROM comment WHERE user_id=${userId} or forum_id in (${userForumsList})`;
      } else {
        dbQuery = `DELETE FROM comment WHERE user_id=${userId}`;
      }
      db.query(dbQuery, (err, result) => {
        // error handler
        if (err) {
          return res.status(400).json({
            sub_err:
              'La suppression de vos commentaires et de votre compte a échouée, veuillez réessayer dans quelques instants.',
            err,
          });
        }

        // delete forums posted by user
        db.query(`DELETE FROM forum WHERE user_id=${userId}`, (err, result) => {
          // error handler
          if (err) {
            return res.status(400).json({
              sub_err:
                'La suppression de vos discussions et de votre compte a échouée, veuillez réessayer dans quelques instants.',
              err,
            });
          }
          ...
          db.query(`SELECT * FROM user WHERE id=${userId}`, (err, result) => {
            ...
            if (err) {
              return res.status(400).json({
                sub_err:
                  'La suppression de votre photo de profil et de votre compte a échouée, veuillez réessayer dans quelques instants.',
                err,
              });
            }
            ...
            if (
              userPic &&
              userPic != 'http://localhost:3000/images/user-icon.png'
            ) {
            ...
            db.query(`DELETE FROM user WHERE id=${userId}`, (err, result) => {
              ...
              if (err) {
                return res.status(400).json({
                  sub_err:
                    'La suppression de votre compte a échouée, veuillez réessayer dans quelques instants.',
                  err,
                });
              }
              // account deleted
